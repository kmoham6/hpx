...
void measureAdjacent_differenceAlgorithms() {
  ...
  // for (size_t s = start; s <= till; s *= 2) {
    std::vector<std::string> arr = generateStrings (start, "string");
    ...
    //  chunk_size *= 2;
    //  hpx::execution::experimental::static_chunk_size scs(s/2);
    ...
    // for (int i = 0; i <= NUM_ITERATIONS + 5; i++) {
      std::vector<std::string> res(start);
      auto t1 = std::chrono::high_resolution_clock::now();
      hpx::adjacent_difference(hpx::execution::seq, arr.begin(), arr.end(),
                               res.begin(), [](const std::string& lhs, const std::string& rhs) {
  ...
  std::cout << "Total time taken: " << duration << " nanoseconds" << std::endl;
    ...
    // }
    // for (int i = 0; i <= NUM_ITERATIONS + 5; i++) {
    //   std::vector<std::string> res1(s);
    //   auto t2 = std::chrono::high_resolution_clock::now();
    //   // hpx::adjacent_difference(hpx::execution::par.with(nc, scs), arr.begin(),
    //   //                          arr.end(), res1.begin());
    //   hpx::adjacent_difference(hpx::execution::par.with(nc, scs), arr.begin(),
    //                            arr.end(), res1.begin(), [](const std::string& lhs, const std::string& rhs) {
    //                        return std::to_string(lhs == rhs);});
